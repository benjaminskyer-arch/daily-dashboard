...
async function getChannelName(channelId) {
  ...
  if (data.ok) {
    return data.channel.is_im ? 'DM' : `#${data.channel.name}`;
  }
  return channelId;
}
...
export async function GET() {
  ...
  async function resolveChannel(channelId) {
    if (!channelCache[channelId]) channelCache[channelId] = await getChannelName(channelId);
    return channelCache[channelId];
  }

  // Get timestamps for last 48 hours
  const since = String(Math.floor(Date.now() / 1000) - 172800);
  ...
  try {
    ...
    if (searchData.ok && searchData.messages?.matches) {
      for (const match of searchData.messages.matches) {
        ...
        // Check if I've replied after this message in the same channel
        let responded = false;
        try {
          if (match.channel?.id) {
        ...
        if (!responded) {
          messages.push({
            ...
            from: fromName,
            channel: channelName,
            text: (match.text || '').replace(/<@[A-Z0-9]+\|([^>]+)>/g, '@$1').replace(/<@[A-Z0-9]+>/g, '@user').replace(/<[^>]+>/g, '').slice(0, 200),
            timestamp: match.ts,
            permalink,
            time: new Date(parseFloat(match.ts) * 1000).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZone: 'America/New_York' }),
          });
        }
      }
    }
    ...
    if (convos.ok && convos.channels) {
      for (const dm of convos.channels) {
        ...
        // Find messages FROM others that I haven't replied to
        ...
        if (!responded) {
          ...
          messages.push({
            ...
            from: fromName,
            channel: 'DM',
            text: (latestFromOther.text || '').replace(/<@[A-Z0-9]+\|([^>]+)>/g, '@$1').replace(/<@[A-Z0-9]+>/g, '@user').replace(/<[^>]+>/g, '').slice(0, 200),
            timestamp: latestFromOther.ts,
            permalink,
            time: new Date(parseFloat(latestFromOther.ts) * 1000).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', timeZone: 'America/New_York' }),
          });
        }
      }
    }
